...
<%
	...
	List postList = postDAO.listPost();
...
<html>
	...
	<body>
		<div id="container">
			<%if(session.getAttribute("id").equals("ksj")) {
					type = "button";%>
			...
			<nav>
				<ul>
					...
					<li><input type="<%=type %>" value="New Post" class="postbtn" onclick="location.href='postlayout.jsp'"></li>
				</ul>
			</nav>
			...
			<section class="space">
				<article>
					<div>
					<div align="center">
						<form action="findpost.jsp" method="post">
						<p>search : 
						<input type="text" placeholder="title" name="ftitle">
						<input type="submit" value="🔍︎"></p></form>
						<table>
						<%if(postList.size() == 0) { %>
							<tr><td><h1>There is no Post</h1></td></tr>
						<%} else { 
								for(int i=0; i<postList.size(); i++) {
									PostBean bean = (PostBean) postList.get(i);
									String title = bean.getTitle();
									String content = bean.getContent();
						%>
							<tr>
							<td><h1><%=i+1 %>.&nbsp;</h1></td>
							<td><h1><a href="detail.jsp?title=<%=bean.getTitle()%>&content=<%=bean.getContent() %>" class="posta" style="text-decoration: none;"><%=bean.getTitle() %></a></h1></td>
							<%if(session.getAttribute("id").equals("ksj")) { %>
								<td><h1><a href="delete.jsp?title=<%=bean.getTitle()%>" class="deletebtndesign">X</a></h1></td>
							<%} %></tr>
						<%
								}
							}
